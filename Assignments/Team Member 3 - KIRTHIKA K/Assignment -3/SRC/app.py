...
def shop():
  ...
  while dictionary != False:
    pro_name.append(dictionary["PRO_NAME"])
    pro_price.append(dictionary["PRO_PRICE"])
    pro_image.append(dictionary["PRO_IMAGE"])
    dictionary = ibm_db.fetch_assoc(stmt)
  ...
  for i in range(0,len(imagearr)):
    imagearr[i]="https://bucket-image-ibmproj.s3.au-syd.cloud-object-storage.appdomain.cloud/"+imagearr[i]
    print("objstorage ", imagearr)
  return render_template('shop.html', len = len(pro_name), pro_name = pro_name, pro_image = pro_image, pro_price = pro_price, imagearr = imagearr)
